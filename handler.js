...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '𝗦𝘂𝗽𝗲𝗿 𝗠𝗶𝗸𝘂𝗕𝗼𝘁-𝗠𝗗') :
...
"thumbnail": apii.data, 
"title": [wm, '💐 𝗦𝘂𝗽𝗲𝗿 ' + gt + ' 🍁', '🌟 centermikubot.gmail.com'].getRandom(),
...
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: lenguajeGB.smsAvisoAG().slice(0,-2), body: [wm, '💐 𝗦𝘂𝗽𝗲𝗿 ' + gt + ' 🍁', '🌟 centermikubot.gmail.com'].getRandom(), thumbnail: gataImg.getRandom(), sourceUrl: [md, nna, nn, nnn, yt, ig, paypal, fb].getRandom() }}}}, tg)
